...
def load_kitti_gt_txt(txt_root, seq):
    ...
    :return: [{anc_idx: *, pos_idx: *, seq: *}]                
     '''
    dataset = []
    file_path = os.path.join(txt_root, 'groundtruths128', '%02d'%seq, 'groundtruths.txt')
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if self.ensure_kpts_num:
                ...
                if kp1_num < len(kp1_w):
                    kp1_w = kp1_w[:kp1_num]
                    score1 = score1[:kp1_num]
                    descs1 = descs1[:kp1_num]
                else:
                    while kp1_num > len(kp1_w):
                        kp1_w = np.vstack((kp1_w[:(kp1_num - len(kp1_w))], kp1_w))
                        score1 = np.hstack((score1[:(kp1_num - len(score1))], score1))
                        descs1 = np.vstack((descs1[:(kp1_num - len(descs1))], descs1))
                
                if kp2_num < len(kp2_w):
                    kp2_w = kp2_w[:kp2_num]
                    score2 = score2[:kp2_num]
                    descs2 = descs2[:kp2_num]
                else:
                    while kp2_num > len(kp2_w):
                        kp2_w = np.vstack((kp2_w[:(kp2_num-len(kp2_w))], kp2_w))
