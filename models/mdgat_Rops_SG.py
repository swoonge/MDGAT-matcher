...
def normalize_keypoints_pointcloud(kpts):
    """ 포인트클라우드 데이터의 키포인트 위치를 정규화 """
    width, height, depth = kpts[:, 0].max() - kpts[:, 0].min(), kpts[:, 1].max() - kpts[:, 1].min(), kpts[:, 2].max() - kpts[:, 2].min()
    one = kpts.new_tensor(1)
    size = torch.stack([one*width, one*height, one*depth])[None]
    center = size / 2
...
class DescriptorEncoder(nn.Module):
    ...
    def __init__(self, feature_dim, layers):
        super().__init__()
        self.encoder = MLP([135] + layers + [feature_dim])
...
class DescriptorGloabalEncoder(nn.Module):
    ...
    def forward(self, kpts):
        inputs = kpts.transpose(1, 2)
        desc = self.encoder(inputs)
        b, dim, n = desc.size()
        #  각 배치와 각 특징 차원별로 전체 키포인트에서 가장 큰 값을 가져옵니다. 이는 각 특징 차원에서 가장 중요한 값을 의미합니다.
        #  다시 말해, 특정 키포인트의 로컬 특징뿐만 아니라, 전체 키포인트에서 가장 중요한 전역 특징 정보를 동시에 포함하게 됩니다.
...
class MDGAT(nn.Module):
    ...
    def infer_mdgat(self, data):
        ...
        desc0 = self.denc(desc0) + self.kenc(kpts0)
        desc1 = self.denc(desc1) + self.kenc(kpts1)
        '''Multi-layer Transformer network.'''
        # point_num = min(desc0.shape[2], desc1.shape[2])
        # k = [point_num//2, None, point_num//2, None, point_num//4, None, point_num//4, None]
    ...
    def forward(self, data):
        ...
        kpts0 = normalize_keypoints_pointcloud(kpts0)
        kpts1 = normalize_keypoints_pointcloud(kpts1)
        ...
        # print("loss mean", loss_mean, " || loss all", loss_all, " || time", time.time()-aa)
