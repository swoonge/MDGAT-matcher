...
def knn(x, src, k):
    '''k개의 가장 가까운 이웃을 찾아 인덱스를 반환합니다.
    
    Args:
        x (torch.Tensor): shape이 (batch_size, num_points, dim)인 입력 텐서입니다.
        src (torch.Tensor): shape이 (batch_size, num_points, dim)인 소스 텐서입니다.
        k (int): 찾을 가장 가까운 이웃의 개수입니다.
    '''
    inner = -2*torch.matmul(x.transpose(2, 1), src) # (batch_size, num_points, num_points)
    xx = torch.sum(x**2, dim=1, keepdim=True) # (batch_size, num_points, 1)
    ss = torch.sum(src**2, dim=1, keepdim=True) # (batch_size, num_points, 1)
    pairwise_distance = -xx.transpose(2, 1) - inner - ss # (batch_size, num_points, num_points)
    idx = pairwise_distance.topk(k=k, dim=-1)[1]   # (batch_size, num_points, k)
    return idx

def get_graph_feature(x, src, k, idx=None):
    '''
    k개의 가장 가까운 이웃을 찾아 인접 행렬을 반환합니다.

    Args:
        x (torch.Tensor): shape이 (batch_size, num_dims, n)인 입력 텐서입니다.
        src (torch.Tensor): shape이 (batch_size, num_dims, m)인 소스 텐서입니다.
        k (int): 찾을 가장 가까운 이웃의 개수입니다.
        idx (torch.Tensor, optional): shape이 (batch_size, num_points, k)인 인덱스 텐서입니다. 기본값은 None입니다.

    Returns:
        torch.Tensor: shape이 (batch_size, n, m)인 인접 행렬입니다.
    '''
...
class MDGAT(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.descriptor == 'FPFH' or self.descriptor == 'FPFH_gloabal':
            ...
            # self.denc(desc0) : MLP(33->128), self.kenc(kpts0, data['scores0']) : MLP(4->128)
